...
def check_ipaddress(secret_dict, input_sourceIp, input_protocol):
    accepted_ip_network_list = lookup(secret_dict, "AcceptedIpNetworkList", input_protocol)
    if not accepted_ip_network_list:
        # No IP provided so skip checks
        print("No IP range provided - Skip IP check")
        return True

    for accepted_ip_network in accepted_ip_network_list.split(','):
        if ip_address(input_sourceIp) in ip_network(accepted_ip_network):
